#Imorting Modules needed, If program is unable to run, download PYQT5.
import sys, math
...
MOVE_MASS = 0
ADD_SPRING = 1
#Spring Class contains force calculations, also works between 2 points and works with spring constant.
class spring:
    ...
    def y_force(self, length):
        return -self.k * (length - self.rest_length)
#Block Class contains positioning, damping to apply (if selected) and if block can move or not.
class block:
    ...
    def damp(self): #Used to apply damping to the simulation (if selected):
        if self.dynamic == True :
...
class mywindow(QWidget):
    ...
    def load(self): #This is used to load a file into the simulation
        options = QFileDialog.Options() #Used to open a file explorer, used to locate the desired file to open.
        options |= QFileDialog.DontUseNativeDialog
    ...
    def __init__(self):
        ...
        panel = QWidget()
        layout2 = QGridLayout()
        #This is the button to start/reset the current simulation that is shown on screen:
        button = QPushButton("Start/Restart")
        button.clicked.connect(self.run)
        layout2.addWidget(button, 1,1,1,1)#Screen positioning        
        #This is the button to pause the current simulation:
        button_2 = QPushButton("Pause")
        button_2.clicked.connect(self.pause)
        layout2.addWidget(button_2, 1,2,1,1)#Screen positioning

        panel.setLayout(layout2)
        layout.addWidget(panel, 1,1,1,2)#Screen positioning
        #This is the button to Remove all masses,anchors and springs from the screen:
        button_3 = QPushButton("Remove All")
        button_3.clicked.connect(self.remove)
        layout.addWidget(button_3, 2,1,1,2)#Screen positioning
        ...
        layout.addWidget(self.damp_edit,3,2,1,1)#Screen positioning
        ...
        layout.addWidget(self.mass_edit,4,2,1,1)#Screen positioning
        #This is the button to add a mass (dynamic/moving) that is shown on screen:
        self.mass_add = QPushButton("Add mass")
        self.mass_add.clicked.connect(self.add_mass)
        layout.addWidget(self.mass_add, 5,1,1,2)#Screen positioning
        #This is the button to add a anchor (static/non moving) that is shown on screen:
        self.anchor_add = QPushButton("Add anchor")
        self.anchor_add.clicked.connect(self.add_anchor)
        layout.addWidget(self.anchor_add, 6,1,1,2)#Screen positioning
        #This is the button to add a spring between two objects (mass/anchor) that is shown on screen:
        self.spring_add = QPushButton("Add Spring")
        self.spring_add.clicked.connect(self.add_spring)
        layout.addWidget(self.spring_add, 7,1,1,2)#Screen positioning

        panel_3 = QWidget()
        layout3 = QGridLayout()
        #This is the button to save the current simulation that is shown on screen:
        self.save_button = QPushButton("Save Simulation")
        self.save_button.clicked.connect(self.save)
        layout3.addWidget(self.save_button,1,1,1,1)#Screen positioning
        #This is the button to load a saved simulation and display it on screen:
        self.load_button = QPushButton("Load Simulation")
        self.load_button.clicked.connect(self.load)
        layout3.addWidget(self.load_button,1,2,1,1)#Screen positioning

        panel_3.setLayout(layout3)
        layout.addWidget(panel_3, 8,1,1,2)#Screen positioning

        self.announcement = QLabel()
        layout.addWidget(self.announcement, 9,1,1,1)#Screen positioning
